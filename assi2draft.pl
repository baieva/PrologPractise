...
result(X):-color([a,b,c],[[a,b],[a,c]],[green,yellow],X).
color([],_,_,[]).
...
%coloring(N,_,[_,H|T],[N,H],2,_,_).
coloring(N,A,C,X,M,List,Mc):-findcolor(Mc,N,A,Cf,M,List,0),coloredf(N,Cf,C,X,Mc).
%coloring(N,A,C,X,M,List,Mc):-findcolor(Mc,N,A,Cf,M,List,0),Cf is M + 1,coloredf(N,Cf,C,X,Mc).
